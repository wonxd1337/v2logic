# main.py - FULL VERSION WITH SMART LEVEL 2 SAMPLING
import os
import sys
import signal
import atexit
...
class MovableTypeMassScanner:
    ...
    ║  - Smart Level 2 Sampling (min 20 domains/IP)              ║
    ...
    def sample_base_quality(self, base_three_octets):
        ...
        Mencari minimal SAMPLE_SIZE IP valid dari base, lalu test reverse IP
        
        Args:
            base_three_octets: contoh "157.7.109."
        
        Returns:
            (is_good, sampled_ips, domain_counts): 
                is_good: True jika semua sample memenuhi syarat (≥20 domain)
        ...
        if len(valid_ips) < Config.SAMPLE_SIZE:
            print(f"\n[SAMPLING] ✗ BASE REJECTED!")
            print(f"[SAMPLING] Only found {len(valid_ips)} valid IPs, need {Config.SAMPLE_SIZE}")
            print(f"[SAMPLING] Base terlalu sepi (sedikit IP valid)")
            return False, valid_ips, {}
        
        print(f"\n[SAMPLING] Found {len(valid_ips)} valid IPs, testing reverse IP...")
        print(f"{'─'*50}")
        ...
        for idx, ip in enumerate(valid_ips[:Config.SAMPLE_SIZE], 1):
            ...
            if self.cache_manager.is_ip_processed(ip):
                ...
                domain_counts[ip] = -1  # Tanda sudah diproses
                continue
            ...
            if domain_count >= Config.MIN_DOMAINS_PER_IP:
                print(f"✓ {domain_count} domains (PASS)")
            else:
                print(f"✗ {domain_count} domains (FAIL - need {Config.MIN_DOMAINS_PER_IP})")
                all_passed = False
                # Tetap lanjut sample untuk report, tapi base sudah gagal
        
        # Tampilkan ringkasan
        print(f"{'─'*50}")
        
        if not all_passed:
            print(f"[SAMPLING] ✗ BASE REJECTED!")
            print(f"[SAMPLING] At least one sample had < {Config.MIN_DOMAINS_PER_IP} domains")
            return False, sampled_ips, domain_counts
        
        # Hitung berapa yang benar-benar pass (exclude cached)
        passed_count = sum(1 for ip in sampled_ips 
                          if domain_counts.get(ip, 0) >= Config.MIN_DOMAINS_PER_IP)
        cached_count = sum(1 for ip in sampled_ips if domain_counts.get(ip, 0) == -1)
        
        # Cached IP dianggap pass karena sudah diproses sebelumnya
        effective_passed = passed_count + cached_count
        
        if effective_passed >= Config.SAMPLE_SIZE:
            print(f"[SAMPLING] ✓ BASE QUALIFIED!")
            print(f"[SAMPLING] {passed_count} samples passed, {cached_count} cached (already processed)")
            return True, sampled_ips, domain_counts
        else:
            print(f"[SAMPLING] ✗ BASE REJECTED!")
            print(f"[SAMPLING] Only {effective_passed}/{Config.SAMPLE_SIZE} passed (including cached)")
            return False, sampled_ips, domain_counts
    ...
    def continuous_scan(self):
        ...
        Level 2 Exploit: Scan FULL hanya jika semua sample ≥20 domains
        """
        print("\n" + "="*60)
        ...
        print(f"[*] Level 2 Exploit: Scan FULL hanya jika semua sample ≥{Config.MIN_DOMAINS_PER_IP} domains")
        ...
        print(f"[*] Sampling threshold: {Config.MIN_DOMAINS_PER_IP} domains per IP")
        ...
        while self.running:
            ...
            # ========== LEVEL 2 EXPLORE: MENCARI BASE BARU (HANYA IP VALID) ==========
                ...
                print(f"Minimum {Config.MIN_DOMAINS_PER_IP} domains required per IP to qualify")
                print(f"{'='*60}")
            ...
            # ========== LEVEL 2 SAMPLING: TEST KUALITAS BASE ==========
                ...
                is_good_base, sampled_ips, domain_counts = self.sample_base_quality(self.current_base_three_octets)
                
                # Tandai sample IP sebagai processed (agar tidak diproses ulang)
                for ip in sampled_ips:
                    if not self.cache_manager.is_ip_processed(ip):
                        status = 'sampled_pass' if domain_counts.get(ip, 0) >= Config.MIN_DOMAINS_PER_IP else 'sampled_fail'
                        self.cache_manager.mark_ip_processed(ip, status)
                
                if not is_good_base:
                    print(f"\n[!] Base {self.current_base_three_octets}x is LOW QUALITY!")
                    print(f"[!] Not all samples met the {Config.MIN_DOMAINS_PER_IP} domains threshold")
                ...
                # BASE BAGUS, lanjut ke exploit mode
                print(f"\n[✓] Base {self.current_base_three_octets}x is HIGH QUALITY!")
                print(f"[✓] All {Config.SAMPLE_SIZE} samples met the threshold!")
                print(f"[✓] Proceeding to EXPLOIT mode (full scan)...")
